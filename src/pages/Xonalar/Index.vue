...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        "№",
        { key: "name", label: "Хоналар", sortable: true },
  ...
  mounted() {
    this.data(),
      this.$root.$on("add_xonalar", xonalar => {
        this.items.push(xonalar);
      });
  },
  methods: {
    RowClicked(item) {
      this.$router.push({ path: "/xonalar-update/" + item.id });
    },
    xonalarLink(id) {
      return this.$router.push({ path: "/xonalar-update/" + id });
    },
    createRoomLink() {
      return this.$router.push({ path: "/xonalar-create" });
    },
    data() {
      let s = this;
      axios({
