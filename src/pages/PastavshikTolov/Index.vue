...
export default {
  ...
  methods: {
    RowClicked(item) {
      this.$router.push({ path: "/provider/pay/update/" + item.id });
    },
    providerPayLink(id) {
      return this.$router.push({ path: "/provider/pay/update/" + id });
    },
    createRoomLink() {
      return this.$router.push({ path: "/provider/pay/create" });
    },
    data() {
      let s = this;
      axios({
        method: "get",
        url: "/pastavchik_pay/all"
      }).then(res => {
        if (res != undefined) {
