...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        "№",
        { key: "name", label: "Дорилар", sortable: true },
        { key: "actions", label: "" }
      ],
      totalRows: 1,
      currentPage: 1,
  ...
  methods: {
    RowClicked(item) {
      this.$router.push({ path: "/dorilar-update/" + item.id });
    },
    dorilarLink(id) {
      return this.$router.push({ path: "dorilar-update/" + id });
    },
    createRoomLink() {
      return this.$router.push({ path: "dorilar-create" });
    },
    data() {
      let s = this;
      axios({
