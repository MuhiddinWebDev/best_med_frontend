...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        "№",
        { key: "user_name", label: "Ф.И.Ш", sortable: true },
        { key: "role", label: "Ҳуқуқи", sortable: true },
        { key: "Room.name", label: "Хона", sortable: true },
  ...
  methods: {
    RowClicked(item) {
      this.$router.push({ path: "/hodimlar-update/" + item.id });
    },
    roomLink(id) {
      return this.$router.push({ path: "hodimlar-update/" + id });
    },
    createRoomLink() {
      return this.$router.push({ path: "hodimlar-create" });
    },
    trash(id) {
