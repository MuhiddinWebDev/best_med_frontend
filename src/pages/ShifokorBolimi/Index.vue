...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        "№",
        { key: "name", label: "Шифокор бўлими", sortable: true },
  ...
  methods: {
    RowClicked(item) {
      this.$router.push({ path: "/shifokor-bolimi-update/" + item.id });
    },
    shifokorBolimiLink(id) {
      return this.$router.push({ path: "shifokor-bolimi-update/" + id });
    },
    createRoomLink() {
      return this.$router.push({ path: "shifokor-bolimi-create" });
    },
    data() {
      let s = this;
      axios({
