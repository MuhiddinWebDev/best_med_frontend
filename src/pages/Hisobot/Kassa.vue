  ...
  <b-container fluid id="tables">
    ...
    <table class="table table-bordered table-striped table-hover table-sm">
      ...
      <tbody v-for="(item, index) in results" :key="index">
        <tr
          class="text-center"
          v-on:dblclick="Clicked(item.doctor_id, item.doctor)"
        >
          <td>{{ index + 1 }}</td>
...
export default {
  ...
  methods: {
    Clicked(id, names) {
      ...
      localStorage.setItem("kassa", id);
      localStorage.setItem("doctor_name", names);
      this.$router.push({ path: "kassa-sverka" });
    },
    Send() {
      let self = this;
      var datas = {
      ...
      axios({
        method: "post",
        url: "registration_arxiv/kassasverka",
        headers: {
