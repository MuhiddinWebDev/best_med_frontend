...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        "№",
        { key: "name", label: "Текширув номи", sortable: true },
        {
          key: "inspector_category.name",
  ...
  methods: {
    RowClicked(item) {
      this.$router.push({ path: "tekshiruvlar-update/" + item.id });
    },

    tekshiruvlarLink(id) {
      return this.$router.push({ path: "tekshiruvlar-update/" + id });
    },
    createRoomLink() {
      return this.$router.push({ path: "tekshiruvlar-create" });
    },
    data() {
      let s = this;
      axios({
