                  ...
                  >
                    <div style="display: flex; margin-right: 15px">
                      <b-button
                        ...
                        @click="tekshiruvBolimiLink(row.item.id)"
                        class="pencil"
                      >
                        <b-icon
                          style="width: 15px;"
...
export default {
  ...
  methods: {
    RowClicked(item) {
      this.$router.push({ path: "/tekshiruv-bolimlari-update/" + item.id });
    },

    tekshiruvBolimiLink(id) {
      return this.$router.push({ path: "tekshiruv-bolimlari-update/" + id });
    },
    createRoomLink() {
      return this.$router.push({ path: "tekshiruv-bolimlari-create" });
    },
    data() {
      let s = this;
      axios({
    ...
    trash(id) {
      ...
      if (trash) {
        ...
        axios({
          method: "delete",
          url: "/inspector_category/delete/" + id
        }).then(res => {
          if (res != undefined) {
            self.data();
          }
