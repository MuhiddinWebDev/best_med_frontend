...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        "№",
        { key: "name", label: "Сўри", sortable: true },
        {
          key: "price",
          label: "Нарх",
          sortable: true,
  ...
  mounted() {
    this.data(),
      this.$root.$on("add_sorilar", sorilar => {
        this.items.push(sorilar);
      });
  },
  methods: {
    RowClicked(item) {
      this.$router.push({ path: "/sori-update/" + item.id });
    },
    soriLink(id) {
      return this.$router.push({ path: "/sori-update/" + id });
    },
    createSoriLink() {
      return this.$router.push({ path: "/sori-create" });
    },
    data() {
      let s = this;
      axios({
