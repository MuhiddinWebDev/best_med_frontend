                  ...
                  >
                    <div style="display: flex; margin-right: 15px">
                      <b-button
                        ...
                        @click="providerLink(row.item.id)"
                        class="pencil"
                      >
                        <b-icon
                          style="width: 15px;"
...
export default {
  ...
  mounted() {
    this.data(),
      this.$root.$on("add_provider", provider => {
        this.items.push(provider);
      });
  },
  methods: {
    RowClicked(item) {
      this.$router.push({ path: "/provider-update/" + item.id });
    },
    providerLink(id) {
      return this.$router.push({ path: "/provider-update/" + id });
    },
    createRoomLink() {
      return this.$router.push({ path: "/provider-create" });
    },
    data() {
      let s = this;
      axios({
