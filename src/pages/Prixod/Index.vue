...
export default {
  ...
  mounted() {
    this.data(),
      this.$root.$on("add_prixod", prixod => {
        this.items.push(prixod);
      });
  },
  methods: {
    RowClicked(item) {
      this.$router.push({ path: "/prixod-update/" + item.id });
    },
    prixodLink(id) {
      return this.$router.push({ path: "/prixod-update/" + id });
    },
    createRoomLink() {
      return this.$router.push({ path: "/prixod-create" });
    },
    data() {
      let s = this;
      axios({
